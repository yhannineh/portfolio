...
const ProjectItem = ({ project }) => {
    ...
    return (
        <div className="projectItem" onClick={toggleExpand} style={{ cursor: 'pointer' }}>
            <img className="projectImage" src={project.icon} alt={project.name}   />
            <h2>{project.name}</h2>
            <div className={itemClass}>
                {isExpanded && (
                    <div>
                        <h2 className="expandedName">{project.name}</h2>
                        <p className="expandedDesc">{project.description}</p>
                        <p className="expandedTechUsed">Technologies Used: {project.technologies.join(', ')}</p>
                        <a className="expandedLink" href={project.url} target="_blank" rel="noopener noreferrer">View Project</a><br/>
